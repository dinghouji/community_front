<template>
  <div class="page-container">
    <div class="action-bar">
      <el-button type="primary" @click="handleAdd"><el-icon><Plus /></el-icon> 新增车位类型</el-button>
    </div>
...
import { getParkingTypePage, saveParkingType, updateParkingType, deleteParkingType } from '@/api/parkingType'
import { ElMessage, ElMessageBox } from 'element-plus'

const loading = ref(false)
const tableData = ref([])
const dialog = reactive({ visible: false, title: '' })
const form = reactive({ id: null, typeName: '', price: 0 })

const getList = async () => {
  loading.value = true
  const res = await getParkingTypePage({ pageNum: 1, pageSize: 20 })
  tableData.value = res.data.records
  loading.value = false
}

const handleAdd = () => {
  Object.assign(form, { id: null, typeName: '', price: 0 })
  dialog.title = '新增车位类型'
  dialog.visible = true
}
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除?', '提示', { type: 'warning' }).then(async () => {
    await deleteParkingType(row.id)
...
const submitForm = async () => {
  if (form.id) {
    await updateParkingType(form)
  } else {
    await saveParkingType(form)
  }
